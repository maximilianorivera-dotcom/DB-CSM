 ...
 *  Connects to n8n webhook: /webhook/health-score
 *  JSON shape: { summary, clients[] }
 * ═══════════════════════════════════════════════════════════════
 */

// ══ CHANGE THIS to your n8n webhook URL ═════════════════════════
const WEBHOOK_URL = "/api/health-score";
// ════════════════════════════════════════════════════════════════

const fmtCLP = n => n != null ? `$${n.toLocaleString("es-CL")}` : "—";

const FRESH = {
  verde:     { bg:"#0b3d2b", fg:"#6ee7b7", dot:"#34d399", label:"Fresco",    desc:"Datos actualizados hace menos de 30 días" },
  amarillo:  { bg:"#3d350b", fg:"#fde68a", dot:"#fbbf24", label:"Tibio",     desc:"Última actualización: 30–60 días" },
  rojo:      { bg:"#3d0b14", fg:"#fca5a5", dot:"#ef4444", label:"Stale",     desc:"Datos con más de 60 días de antigüedad" },
  sin_fecha: { bg:"#252525", fg:"#9ca3af", dot:"#6b7280", label:"Sin fecha", desc:"No hay fecha de referencia disponible" },
};
...
function Tooltip({ children, content, width = 280 }) {
  ...
  return (
    <span onMouseEnter={handleEnter} onMouseLeave={() => setShow(false)} style={{ position:"relative", cursor:"default" }}>
      {children}
      {show && (
        <div style={{ position:"fixed", left:pos.x, top:pos.y, transform:"translate(-50%, -100%)", width, background:"#1a1a28", border:"1px solid #2d2d44", borderRadius:10, padding:"14px 16px", zIndex:9999, boxShadow:"0 12px 40px rgba(0,0,0,0.6)", pointerEvents:"none" }}>
          {content}
          <div style={{ position:"absolute", bottom:-6, left:"50%", transform:"translateX(-50%) rotate(45deg)", width:12, height:12, background:"#1a1a28", borderRight:"1px solid #2d2d44", borderBottom:"1px solid #2d2d44" }}/>
        </div>
      )}
    </span>
  );
...
function IndexTooltip({ c }) {
  ...
  return (
    <div style={{ fontFamily:"'Outfit',sans-serif", fontSize:12, color:"#d1d5db" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
        ...
        {hasLoy && c.loyalty_reason && (
          <div style={{ fontSize:10, color:"#7c7c8a", marginTop:-4, paddingLeft:4 }}>↳ {c.loyalty_reason}</div>
        )}
        <div style={{ display:"flex", justifyContent:"space-between" }}>
        ...
        {hasFb && c.feedback_source && (
          <div style={{ fontSize:10, color:"#7c7c8a", marginTop:-4, paddingLeft:4 }}>↳ Fuente: {c.feedback_source}</div>
        )}
      </div>
...
function MomentumTooltip({ c }) {
  ...
  return (
    <div style={{ fontFamily:"'Outfit',sans-serif", fontSize:12, color:"#d1d5db" }}>
      ...
      {bar("Variación convos %", c.variacion_pct, 80, (c.variacion_pct??0) >= 0 ? "#4ade80" : "#f97316")}
  ...
  return (
    <div style={{ fontFamily:"'Outfit',sans-serif", fontSize:12, color:"#d1d5db" }}>
      ...
      <div style={{ fontSize:10, color:"#6b7280", lineHeight:1.5 }}>
        Indica qué tan recientes son los datos de Loyalty y Feedback que componen el Index. Se mide desde la fecha más reciente entre ambos inputs.
      </div>
      ...
      <div style={{ width:36, height:36, position:"relative" }}>
        <svg width={36} height={36} viewBox="0 0 36 36" style={{ transform:"rotate(-90deg)" }}>
          ...
          <circle cx={18} cy={18} r={14} fill="none" stroke={color} strokeWidth={3}
            strokeDasharray={`${87.96*pct} ${87.96*(1-pct)}`} strokeLinecap="round"
...
export default function HealthDashboard() {
  ...
  const clients = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case "freshness": { const o={verde:4,amarillo:3,rojo:2,sin_fecha:1}; va=o[a.freshness]||0; vb=o[b.freshness]||0; break; }
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh", background:"#08080d", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column" }}>
      ...
      <p style={{ color:"#6b7280", marginTop:16, fontFamily:"'JetBrains Mono',monospace", fontSize:12 }}>Cargando Health Score…</p>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
    </div>
  );

  if (error) return (
    <div style={{ minHeight:"100vh", background:"#08080d", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:12 }}>
      <div style={{ color:"#ef4444", fontSize:14, fontFamily:"'JetBrains Mono',monospace" }}>Error al cargar datos</div>
      <div style={{ color:"#6b7280", fontSize:12 }}>{error}</div>
      <div style={{ color:"#4b5563", fontSize:11, marginTop:8 }}>Endpoint: {WEBHOOK_URL}</div>
    </div>
  );
  ...
  const SortHeader = ({ label, sortId, align="center" }) => {
    ...
    return (
      <th onClick={() => handleSort(sortId)} style={{ ...S.th, textAlign:align, cursor:"pointer", userSelect:"none", color:active?"#c9a96e":"#6b7280", transition:"color 0.2s" }}>
        {label}<span style={{ marginLeft:4, fontSize:9, opacity:active?1:0 }}>{sortDir==="desc"?"▼":"▲"}</span>
      </th>
    );
  };
  ...
  return (
    <div style={S.root}>
      ...
      <aside style={S.sidebar}>
        ...
        <div style={{ padding:"0 12px 8px", fontSize:10, color:"#4b5563", fontFamily:"'JetBrains Mono',monospace", textTransform:"uppercase", letterSpacing:0.5 }}>
          Success Managers
        </div>
          ...
          {csms.map((csm, i) => {
            ...
            return (
              <button key={csm.name} onClick={() => setSelectedCSM(csm.name)} style={{
                ...S.csmBtn, background:active?"#141420":"transparent",
                borderLeft:active?"2px solid #c9a96e":"2px solid transparent",
                animation:`slideIn 0.3s ease ${i*0.04}s both`,
              }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <span style={{ fontWeight:active?600:400, color:active?"#e5e7eb":"#9ca3af", fontSize:13 }}>{csm.name}</span>
                  <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:10, color:"#6b7280", background:"#16162a", padding:"1px 6px", borderRadius:4 }}>{csm.count}</span>
                </div>
                <div style={{ fontSize:11, color:"#4b5563", fontFamily:"'JetBrains Mono',monospace", marginTop:2 }}>{fmtCLP(csm.mrr)}</div>
              </button>
            );
          })}
        </nav>
        <div style={S.sidebarFooter}>
          {summary && (
            <div style={{ fontSize:10, color:"#4b5563", fontFamily:"'JetBrains Mono',monospace", lineHeight:1.6 }}>
              <div>{summary.totalClients} cuentas · {fmtCLP(summary.totalMRR)}</div>
              <div>{new Date(summary.generatedAt).toLocaleString("es-CL", { day:"2-digit", month:"short", hour:"2-digit", minute:"2-digit" })}</div>
            </div>
          )}
        </div>
      </aside>
      ...
      <main style={S.main}>
        ...
        <div style={S.tableWrap}>
          <table style={S.table}>
            ...
            <tbody>
              {clients.map((c, i) => (
                <tr key={c.client_id} style={{ ...S.tr, animation:`fadeIn 0.3s ease ${i*0.025}s both` }}
                  ...
                  <td style={{ ...S.td, maxWidth:240 }}>
                    ...
                    <div style={{ fontSize:10, color:"#4b5563", marginTop:1 }}>
                      {c.plan} · {c.country}
                      {c.es_moroso && <span style={{ color:"#ef4444", marginLeft:6 }}>● moroso</span>}
                      {c.cant_churn > 0 && <span style={{ color:"#f97316", marginLeft:6 }}>↺{c.cant_churn}</span>}
                    </div>
                  </td>
                  <td style={{ ...S.td, textAlign:"right", fontFamily:"'JetBrains Mono',monospace", fontSize:13, fontWeight:500, color:"#e5e7eb" }}>
                    {fmtCLP(c.mrr)}
                    {c.mrr_var !== 0 && c.mrr_var != null && (
                      <div style={{ fontSize:10, color:c.mrr_var>0?"#4ade80":"#f97316", marginTop:1 }}>
                        {c.mrr_var > 0 ? "+" : ""}{fmtCLP(c.mrr_var)}
                      </div>
                    )}
                  </td>
                  <td style={{ ...S.td, textAlign:"center" }}>
                    <Tooltip content={<IndexTooltip c={c}/>} width={260}><IndexDot value={c.index_score}/></Tooltip>
                  </td>
                  <td style={{ ...S.td, textAlign:"center" }}>
                    <Tooltip content={<MomentumTooltip c={c}/>} width={300}><MomentumBadge score={c.momentum_score} symbol={c.momentum_symbol} label={c.momentum_label}/></Tooltip>
                  </td>
                  <td style={{ ...S.td, textAlign:"center" }}>
                    <Tooltip content={<FreshnessTooltip c={c}/>} width={280}><FreshBadge freshness={c.freshness}/></Tooltip>
                  </td>
                </tr>
              ))}
              {clients.length === 0 && (
...
const S = {
  root: { display:"flex", minHeight:"100vh", background:"#08080d", color:"#e5e7eb", fontFamily:"'Outfit',sans-serif" },
  sidebar: { width:240, minWidth:240, background:"#0b0b12", borderRight:"1px solid #16162a", display:"flex", flexDirection:"column", position:"sticky", top:0, height:"100vh", overflowY:"auto" },
  sidebarHeader: { padding:"20px 16px 12px", display:"flex", alignItems:"center", gap:8 },
  csmBtn: { width:"100%", padding:"10px 12px", border:"none", borderRadius:6, cursor:"pointer", textAlign:"left", marginBottom:2, transition:"all 0.15s", display:"block", fontFamily:"inherit" },
  sidebarFooter: { padding:"12px 16px", borderTop:"1px solid #16162a" },
  main: { flex:1, padding:"24px 32px", overflowY:"auto", maxHeight:"100vh" },
  header: { display:"flex", justifyContent:"space-between", alignItems:"flex-end", marginBottom:20, paddingBottom:16, borderBottom:"1px solid #16162a" },
  title: { margin:0, fontSize:24, fontWeight:700, color:"#f3f4f6", letterSpacing:-0.3 },
  subtitle: { margin:"4px 0 0", fontSize:13, color:"#6b7280", fontFamily:"'JetBrains Mono',monospace" },
  kpi: { background:"#0e0e18", borderRadius:8, padding:"10px 16px", minWidth:100, textAlign:"center" },
  search: { background:"#0e0e18", border:"1px solid #1e1e32", borderRadius:8, padding:"8px 14px", color:"#e5e7eb", fontSize:13, width:260, fontFamily:"'Outfit',sans-serif", outline:"none" },
  tableWrap: { borderRadius:10, border:"1px solid #16162a", overflow:"hidden" },
  table: { width:"100%", borderCollapse:"collapse" },
  th: { padding:"12px 16px", fontSize:10, fontFamily:"'JetBrains Mono',monospace", textTransform:"uppercase", letterSpacing:0.8, background:"#0b0b12", borderBottom:"1px solid #16162a", fontWeight:500, whiteSpace:"nowrap" },
  tr: { borderBottom:"1px solid #0e0e18", transition:"background 0.15s", cursor:"default" },
