          ...
          <div className="flex flex-wrap gap-2">
            {npsRoles.map(r=>(
              <button key={r.id} onClick={()=>setNpsRole(r.id)}
                ...
                {r.label}{r.id!=="global"&&roleEntries[r.id].length>0&&<span className="ml-1 opacity-60">({roleEntries[r.id].length})</span>}
              </button>
            ))}
          </div>
        </div>
      ...
      {npsRole==="global"?(
        c.npsG===null
          ...
          :<div className="space-y-2">
            ...
            {c.npsComments.length>0&&<div className="space-y-2 mt-3">
              ...
              {c.npsComments.slice(0,5).map((cm,i)=><div key={i} className="p-3 bg-gray-50 rounded-lg"><p className="text-sm text-gray-700">"{cm}"</p></div>)}
            </div>}
          </div>
      ):(
        roleEntries[npsRole].length===0?(
          <div className="py-8 text-center border-2 border-dashed border-gray-100 rounded-xl">
            ...
            <p className="text-xs text-gray-300 mt-1.5">Requiere <code className="bg-gray-100 px-1.5 py-0.5 rounded text-gray-400">nps.byRole.{npsRole}</code> en el webhook</p>
          </div>
        ):(
            ...
            <div className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span className={`text-2xl font-bold ${avgCurrent>=8?"text-emerald-600":avgCurrent>=6?"text-amber-500":"text-red-600"}`}>{avgCurrent!==null?avgCurrent.toFixed(1):"—"}</span>
    ...
    <div className="grid grid-cols-4 gap-4 items-start">
      ...
      <div className="col-span-1 space-y-3">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-2">
          ...
          {c.npsG!==null&&<Block label="NPS Global" value={`${c.npsG}/10`} extra={c.npsPrev!==null?<Delta value={c.npsG-c.npsPrev} suffix=" pts" size="xs"/>:null} tooltip="Net Promoter Score global"/>}
        </div>
